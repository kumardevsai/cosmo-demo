...
function batch_update() {
	...
	for (var i = 1; i < table.rows.length; i++) {
		...
		// 查找新添加的行
		if (!(row.getAttribute("isNew") || row.getAttribute("Update") === "Y"))
			continue;
		// 插入值
		var sqlvalue = '';
		// 行数据是否存在
		...
		for (var j = 0; j < row.cells.length; j++) {
			if (j === 1)
				updateId = cell.children[0].getAttribute("DataValue");
			var cell = row.cells[j];
			// 是否是新添加的字段值
			if (typeof(cell.NewValue) != "undefined") {
				// 未获取到插入的字段
				if (flag === false && row.getAttribute("isNew")) {
					// 字段拼接
					sqlfield += attr(cell, 'Field') + ',';
				}
				...
				if (row.getAttribute("Update") === "Y" && !row.getAttribute("isNew")) {
					sqlfieldUpdate += attr(cell, "Field") + "='" + cell.NewValue + "',";
				}
				exsitData = true;
			}
		}
		...
		sqlvalue = sqlvalue.substring(0, sqlvalue.length - 1);
		if (row.getAttribute("Update") === "Y" && !row.getAttribute("isNew")) {
			if (sqlfieldUpdate.lastIndexOf(",") !== -1)
				sqlfieldUpdate = sqlfieldUpdate.substring(0, sqlfieldUpdate.lastIndexOf(","));
			// 插入语句
			sqls += "<sql>update " + unescape(document.getElementById('table0').DataSource) + " set " + sqlfieldUpdate + " where id='" + updateId + "'</sql>";
			// 删除属性
			row.removeAttribute("Update");
			// 添加数组
			updateRows.push(row);
	...
	if (xml.indexOf("错误:") != -1) {
		...
		for (var i = 0; i < addRows.length; i++) {
			addRows[i].setAttribute("isNew", true);
		}
		for (var i = 0; i < addRows.length; i++) {
			addRows[i].setAttribute("Update", "Y");
		}
		return;
	} else {
		// 更新
