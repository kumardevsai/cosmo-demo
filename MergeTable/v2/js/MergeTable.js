...
MergeTable.restrict = {
	...
	operation: {
					...
					for (var j = 0; j < selection2ArrayStack[i].length; j++) {
						...
						if (MergeTable.restrict.persist.storage[y][x]) {
							if (MergeTable.restrict.options.retainMergeText)
								text += MergeTable.restrict.persist.storage[y][x].innerHTML.Trim();
							if (j === 0) {
					...
					for (var j = 0; j < selection2ArrayStack[i].length; j++) {
						...
						if (MergeTable.restrict.persist.storage[y][x]) {
							if (i === 0 && j === 0) {
								...
								if (MergeTable.restrict.options.retainMergeText)
									MergeTable.restrict.persist.storage[y][x].innerHTML = text.Trim();
		...
		splitV: function() {
			if (MergeTable.restrict.operation.checkSplitV()) {
				...
				MergeTable.util.attachEvent(insertCell);
	...
	persist: {
		...
		clear: function() {
			MergeTable.restrict.persist.storage = [];
			MergeTable.restrict.persist.place = [];
			MergeTable.restrict.persist.selection = [];
			MergeTable.restrict.persist.edition = [];
			MergeTable.restrict.persist.range = {};
	...
	plugin: {
		...
		edit: function() {
			var editTextArea = document.getElementById("editTextArea");
			MergeTable.restrict.reloadStr(editTextArea.value);
			editContent.style.display = "none";
		},
		contentEditable: function(ele, index) {
			...
			MergeTable.restrict.persist.edition = [];
			// 将当前显示的文本框添加到数组缓存中
			MergeTable.restrict.persist.edition.push([index, textarea]);
		},
		clearEditable: function() {
			for (var i = 0; i < MergeTable.restrict.persist.edition.length; i++) {
				var textarea = MergeTable.restrict.persist.edition[i][1];
	...
	init: function() {
		var tableContainer = document.getElementById("tableContainer");
		var table = tableContainer.firstElementChild || tableContainer.firstChild;
		var rows = table.rows;
		for (var i = 0; i < rows.length; i++) {
			...
			for (var j = 0; j < rows[i].cells.length; j++) {
				...
				MergeTable.util.attachEvent(cell);
	...
	reloadStr: function(str) {
		MergeTable.restrict.persist.clear();
		MergeTable.restrict.loadStr(str);
		MergeTable.restrict.init();
	}
};
...
function onCellMouseOver(e) {
	...
	if (tagNameV === "td" || tagNameV === "th") {
		if (MergeTable.restrict.persist.mouse.status === 0) {
			...
			MergeTable.restrict.plugin.clearEditable();
		}
	}
};
...
function onCellMouseDown(e) {
	...
	if (tagName === "td" || tagName === "th") {
		...
		MergeTable.restrict.plugin.contentEditable(ele, index);
	}
};

AttachEvent(window, "load", MergeTable.restrict.init(), false);