...
var MergeTable = window.MergeTable = (function() {
	...
	function AffixSingleColumn(evt) {
		...
		var cellindex = evt.parentNode.cellIndex;
		var rowindex = utils.fixedRowIndex(evt.parentNode.parentNode);

		for (var j = 0; j < valArrayLen; j++) {
			if (valArray[j] == "") continue;
			var arry = valArray[j].split("\t");
			var k = 0;
			var row = utils.fixedTableRows(evt.parentNode.parentNode.parentNode.parentNode)[rowindex + j];
			if (row == null) break;
			var idx = cellindex;
	...
	function findRowsBySelection(selection) {
		...
		var rows = [];
		for (var i = 0; i < selection2ArrayStack.length; i++) {
			var row = getRow(utils.index2Obj(selection2ArrayStack[i][0]).y);
			if (row)
				rows.push(row);
		}
		return rows;
	...
	function trans2Head() {
		...
		if (!thead) {
			...
			var table = utils.firstChild(document.getElementById(id));
	...
	function trans2Th() {
		...
		for (var i = 0; i < selection2ArrayStack.length; i++) {
			for (var j = 0; j < persist.storage[i].length; j++) {
				var ocell = persist.storage[utils.index2Obj(selection2ArrayStack[i][0]).y][j];
				if (ocell) {
					...
					persist.storage[utils.index2Obj(selection2ArrayStack[i][0]).y][j] = cell;
					utils.attachEvent(cell);
				}
			}
		}
	...
	function setTbodyType(type) {
		...
		removeNullRows();
		var selection2ArrayStack = selectionTrans2ArrayStack(selection);
		var start = utils.index2Obj(selection2ArrayStack[0][0]).y;
		var end = utils.index2Obj(selection2ArrayStack[selection2ArrayStack.length - 1][0]).y;
		var offsetY = utils.fixedRowIndex(tbody.children[0]);
		...
		for (var i = 0; i < selection2ArrayStack.length; i++) {
			for (var j = 0; j < selection2ArrayStack[i].length; j++) {
				...
				if (persist.storage[obj.y][obj.x]) {
					...
					persist.backupAttrs[obj.y + defaults.separator + obj.x] = new PersistAttr(persist.backupAttrs[obj.y + defaults.separator + obj.x].css, persist.storage[obj.y][obj.x].getAttribute("td_type"), persist.storage[obj.y][obj.x].className);
				}
			}
		}
		clearSelection();
	...
	function triggerOnCellMouseUp() {
		var obj = utils.index2Obj(persist.range.end);
		if (obj) {
			var cell = persist.storage[obj.y][obj.x];
			if (cell)
