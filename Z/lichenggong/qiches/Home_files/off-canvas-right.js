 ...
 !function($){
 	...
 	$(document).ready(function(){
		...
		if ($.support.t3transform !== false) {
			...
			$nav = $('<div class="t3-mainnav-right"><div class="t3-mainnav-right-map"></div></div>').appendTo($('<div id="off-canvas-nav-right"></div>').appendTo(document.body));
			...
			**/
			...
			showNav = function () {
				...
				setTimeout (function(){
					...
					$('html').addClass ('off-canvas-enabled-right');
					$(window).bind('scroll touchmove', posNav);

					// hide when click on off-canvas-nav
					$('#off-canvas-nav-right').bind ('click', function (e) {
						e.stopPropagation();
					});
					
					$('#off-canvas-nav-right a').bind ('click', hideNav);
			...
			hideNav = function (e) {
				...
				$('#off-canvas-nav-right').unbind ('click');
				$('#off-canvas-nav-right a').unbind ('click', hideNav);
				$('body').unbind ('click', bdHideNav);
				
				$('html').removeClass ('off-canvas-enabled-right');
