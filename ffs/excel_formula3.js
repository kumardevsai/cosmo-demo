...
var ExcelFormula = window.ExcelFormula = (function() {
	...
	**/
	var excution = (function() {
		...
		// round 等函数的参数处理
		function roundParamsProcessor(arg, a, b, c, d, flag) {
			// 获取参数数组
			...
			// 舍位参数不可能为小数
			// flag 为true表示舍位参数可以为小数
			if ((d.v.point !== undefined || d.v.num_digits !== undefined) && flag !== true) {
		...
		function ceilingHandler(a, b, c, d) {
			...
			if (/\./.test(mo)) {
				if (c.mark === "-")
					mo = excution.rounddown(mo + "," + 0);
				else
					mo = excution.roundup(mo + "," + 0);
				if (mo.flag === true)
					result = mo.value * b;
			} else {
				result = mo.value * b;
