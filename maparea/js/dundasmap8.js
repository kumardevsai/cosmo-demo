var DundasMap = (function(DundasMapMo) {
	var compatMo = DundasMapMo ? DundasMapMo.MapManager.compatMo : false; 
	return {
		MapManager: function() {
			...
			this.addImageMap = function(image) {
				if (image.imageMapText != null) {
					...
					if (DundasMapMo)
						DundasMapMo.MapManager.caculateExElesPosition(image, imageMap);
				}
			}
			...
			this.recreateGridSections = function() {
				if (this.viewport != null) {
					...
					if(DundasMapMo)
						DundasMapMo.MapManager.destroyExEles();
				}
			}
	...
	if (typeof(DundasMapMo) !== 'undefined')
		return DundasMapMo;
	else
		return null;
}());