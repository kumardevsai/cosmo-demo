 var DundasMap = {
     MapManager: function() {
         ...
         this.areaHTMLDef = (function() {
             ...
             // 获取元素
             function getExElesArr(areaHTML) {
                 var arr = eval(areaHTML);
                 return arr;
             };

             // 计算热区显示元素的位置
             function caculateAreaElesPosition(img, imageMap) {
                 var zoom = that.currentZoom;
                 ...
                 for (var i = 0; i < mapAreas.length; i++) {
                     ...
                     if (areaHTML === null)
                         continue;
                     var _obj;
                     var arr = getExElesArr(areaHTML);
                     for (var i = 0; i < arr.length; i++) {
                         var min_zoom, max_zoom;
                         if (arr[i].zoom) {
                             min_zoom = arr[i].zoom[0];
                             max_zoom = arr[i].zoom[1];
                         }
                         min_zoom = min_zoom ? min_zoom : 20;
                         max_zoom = max_zoom ? max_zoom : 20000;
                         if (min_zoom <= zoom && max_zoom >= zoom) {
                             _obj = arr[i];
                             break;
                         }
                     }
                     ...
                     areaElement.innerHTML = _obj.html;
                     // 获取偏移
                     var areaPosition = getAreaPosition(_obj.position);
                     // 显示到页面
