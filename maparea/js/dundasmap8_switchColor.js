...
var DundasMap = {
    MapManager: function() {
        this.initialize = function() {
            ...
            if (window.addEventListener) {
                ...
                this.viewport.addEventListener("touchmove", function() {
                    _this.onDragStart(event)
                }, false);
                this.zoomPanel.addEventListener("touchstart", function() {
                    _this.onZoomPanelMouseDown(event)
                }, false);
                this.zoomPanel.addEventListener("touchmove", function() {
                    _this.onDragStart(event)
                }, false);
                //touch里的面板没有触发click事件，添加面板click事件
                this.zoomPanel.addEventListener("touchend", function() {
                    _this.onZoomPanelClick(event, _this)
                }, false);
            }
            //hxadd end
        ...
        this.onClick = function(e) {
            ...
            if (window.event.touches && (window.event.touches.length < 2)) {
                e.layerX = e.touches[0].pageX;
                e.layerY = e.touches[0].pageY;
                e.target = e.target;
            };
            //hxadd end
        ...
        this.updateVisibleSections = function(commandName, commandArgument, async) {
            ...
            var minVisibleX = Math.floor(-contentOffsetX / this.gridSectionSizeWidth) + 1; //**1
            minVisibleX = Math.max(0, minVisibleX);
            minVisibleX = Math.min(this.gridSectionsXCount - 1, minVisibleX);
            var minVisibleY = Math.floor(-contentOffsetY / this.gridSectionSizeHeight) + 1; //**1
        ...
        this.findImageMap = function(imageMapName) {
            for (var i = 0; i < this.viewport.childNodes.length; i++) {
        ...
        this.getViewParams = function() {
            ...
            viewCenterX /= contentWidth / 100.0;
            viewCenterY /= contentHeight / 100.0;
            return "&_viewCenterX=" + viewCenterX + "&_viewCenterY=" + viewCenterY + "&_zoom=" + zoom + "&_gridOffsetX=" + this.gridSectionsOffsetX + "&_gridOffsetY=" + this.gridSectionsOffsetY;
        }
        this.getClickCallbackUrl = function(x, y) {
            var requestParams = "_mapControlID=" + this.mapControlId + "&_controlPersistence=" + this.controlPersistence + "&_clickX=" + x + "&_clickY=" + y;
            if (this.viewport != null) {
        ...
        this.getFullUpdateCallbackUrl = function(commandName, commandArgument) {
            var requestParams = "_mapControlID=" + this.mapControlId + "&_fullUpdate=1" + "&_controlPersistence=" + this.controlPersistence;
            if (this.viewport != null) {
                requestParams += this.getViewParams();
            }
            if (commandName != null) {
        ...
        this.getContentOnlyCallbackUrl = function(gridX, gridY, commandName, commandArgument, imageMap) {
            var requestParams = "_mapControlID=" + this.mapControlId + "&_controlPersistence=" + this.controlPersistence + "&_imageUrl=" + encodeURIComponent(this.imageUrl) + "&_gridX=" + gridX + "&_gridY=" + gridY;
            if (this.viewport != null) {
                requestParams += this.getViewParams();
            }
            if (commandName != null) {
        ...
        this.addImageMap = function(image) {
            if (image.imageMapText != null) {
                ...
                // 创建形状颜色显示
                this.shapeColorsDef.getShapePaths(image, imageMap);
            }
        }
        ...
        this.recreateGridSections = function() {
            if (this.viewport != null) {
                ...
                // 地图颜色显示删除
                this.shapeColorsDef.destoryShapePaths();
            }
        }
        this.resetCachedSections = function() {
            ...
            for (var i = 0; i < this.contentFiller.childNodes.length; i++) {
        ...
        this.onMouseDown = function(e) {
            ...
            if (window.addEventListener) {
                ...
                window.addEventListener(mousemove, this.onMouseMove, false); //改动事件
                window.addEventListener(mouseup, this.onMouseUp, false); //改动事件
        ...
        this.onMouseUp = function(e) {
            ...
            if (window.__thisManager) {
                thisManager = window.__thisManager;
                window.removeEventListener(mousemove, thisManager.onMouseMove, false); //改动事件
                window.removeEventListener(mouseup, thisManager.onMouseUp, false); //改动事件
        ...
        this.getZoomPanelThumbRect = function() {
            ...
            if (this.zoomPanelOrientation == "Horizontal") {
                ...
                result = new DundasMap.Rectangle(thumbPosition, scaleRect.y + scaleRect.height / 2, this.zoomPanelPointerWidth / 100 * scaleRect.height, this.zoomPanelPointerLength / 100 * scaleRect.height);
            } else {
                ...
                result = new DundasMap.Rectangle(scaleRect.x + scaleRect.width / 2, thumbPosition, this.zoomPanelPointerLength / 100 * scaleRect.width, this.zoomPanelPointerWidth / 100 * scaleRect.width);
            }
        ...
        this.getPossibleZoomValues = function() {
            ...
            for (var i = 0; i < this.zoomPanelTickCount; i++) {
        ...
        this.onZoomPanelMouseDown = function(e) {
            ...
            if (e.touches && (e.touches.length < 2)) {
                ...
                while (__target != this.mapDiv) {
                    Left += __target.offsetLeft;
                    Top += __target.offsetTop;
                    __target = __target.parentNode;
                }
                layerX = e.pageX - Left;
                layerY = e.pageY - Top;
            }
            if (this.getZoomPanelThumbRect().contains(layerX, layerY)) {
                this.mouseDownX = e.clientX;
                this.mouseDownY = e.clientY;
                ...
                if (window.addEventListener) {
                    window.__thisManager = this;
                    window.__zoomPanel = this.zoomPanel; //hxadd
                    window.addEventListener(mousemove, this.onZoomPanelMouseMove, false); //改动事件
                    window.addEventListener(mouseup, this.onZoomPanelMouseUp, false); //改动事件
        ...
        this.onZoomPanelMouseMove = function(e) {
            ...
            if (e.touches && (e.touches.length < 2)) {
                e = e.touches[0];
                layerX = e.pageX - window.__zoomPanel.offsetLeft;
                layerY = e.pageY - window.__zoomPanel.offsetTop;
            }
            var thisManager = null;
            if (window.__thisManager) {
                thisManager = window.__thisManager;
        ...
        this.onZoomPanelClick = function(e) {
            ...
            var layerX, layerY, __target, _zoomPanel = this.zoomPanel;
            if (window.event && (!window.event.touches)) {
                e = window.event;
                layerX = e.offsetX;
                layerY = e.offsetY;
                __target = e.srcElement;
            } else {
                layerX = e.layerX;
                layerY = e.layerY;
                __target = e.target;
            ...
            if (e.touches && (e.touches.length < 2)) {
                ...
                __target = e.target;
                layerX = e.pageX - parseInt(_zoomPanel.style.left);
                layerY = e.pageY - parseInt(_zoomPanel.style.top);
                if (arguments[1].getZoomPanelRect(true).contains(layerX, layerY)) {
                    this.incrementZoom(true);
                    return true;
                }
                if (arguments[1].getZoomPanelRect(false).contains(layerX, layerY)) {
                    this.incrementZoom(false);
                    return true;
                }
            } else if (!e.touches) {
                while (__target != _zoomPanel) {
                    layerX += __target.offsetLeft;
                    layerY += __target.offsetTop;
                    __target = __target.parentNode;
                }
        ...
        this.shapeColorsDef = (function() {
            ...
            (function setRaphael() {
                var r_i = setInterval(function() {
                    if (Raphael) {
                        R = Raphael(that.contentFiller, that.contentFiller.offsetWidth, that.contentFiller.offsetHeight);
                        if (R.canvas) {
                            R.canvas.style.left = 0;
                            R.canvas.style.zIndex = 1;
                        }
                        R.left == 0;
                        clearInterval(r_i);
                    }
            ...
            // 热区coords路径转换为svg绘线路径
            function mapArea2SvgPath(path, offsetLeft, offsetTop) {
                var arr = path.split(',');
                var p = ' M ' + (parseInt(arr.shift()) + offsetLeft) + ',' + (parseInt(arr.shift()) + offsetTop);
                var len = arr.length;
                for (var i = 0; i < len; i += 2) {
                    p += ' L ' + (parseInt(arr[i]) + offsetLeft) + ',' + (parseInt(arr[i + 1]) + offsetTop);
                }
                return p;
            };
            ...
            function getShapePaths(img, imageMap) {
                // 获取contentFiller
                var contentFillerDiv = img.parentNode;
                // 获取热区
                var mapAreas = imageMap.children;
                var offsetLeft = parseInt(img.style.left);
                var offsetTop = parseInt(img.style.top);
                for (var i = 0; i < mapAreas.length; i++) {
                    var area = mapAreas[i];
                    // 获取要显示的内容
                    var shapeColors = area.getAttribute("shapeColors");
                    if (shapeColors === null)
                        continue;
                    var switchTime = area.getAttribute("switchTime");
                    switchTime = switchTime ? switchTime : 50;
                    var shapeGroup = area.getAttribute("shapeGroup");
                    var startColor = area.getAttribute("startColor");
                    var p = R.path(mapArea2SvgPath(area.getAttribute("coords"), offsetLeft, offsetTop));
                    var p = p.attr(attr).attr({
                        fill: startColor ? startColor : '#FFFFFF'
                    });
                    pushPaths(shapeGroup, p, shapeColors, switchTime, startColor);
                }
            };
            ...
            function pushPaths(shapeGroup, path, shapeColors, switchTime, startColor) {
                if (!aus[shapeGroup]) {
                    var shapePath = new ShapePath();
                    shapePath.shapeColors = shapeColors;
                    shapePath.switchTime = switchTime;
                    shapePath.shapeGroup = shapeGroup;
                    shapePath.startColor = startColor;
                    aus[shapeGroup] = shapePath;
                }
                aus[shapeGroup].paths.push(path);
            };

            function destoryShapePaths() {
                for (var i in aus) {
                    for (var j = 0; j < aus[i].paths.length; j++) {
                        aus[i].paths[j].remove();
                    }
                }
                aus = [];
            };

            function switchShapePathColors() {
                for (var i in aus) {
                    for (var j = 0; j < aus[i].paths.length; j++) {
                        aus[i].paths[j].color = Raphael.getColor();
                        (function(st) {
                            switchBgColor(st, R, aus[i].shapeColors, aus[i].switchTime);
                        })(aus[i].paths[j]);
                    }
                }
            };
            return {
                getShapePaths: getShapePaths,
                destoryShapePaths: destoryShapePaths,
                switchShapePathColors: switchShapePathColors
            };
        })();
    },
    Point: function(x, y) {
    ...
    LinkedList: function(gridSectionManager) {
        ...
        this.swapBuffer = function(image, x, y) {
            ...
            if (this.gridSectionManager.gridSections[x] != null && this.gridSectionManager.gridSections[x][y] == image) {
    ...
    ListNode: function(obj, gridXParam, gridYParam, zoom, gridSectionManager) {
        ...
        this.isVisible = function() {
            return (this.zoom == this.gridSectionManager.getZoom() && this.gridSectionManager.currentVisibleBounds.contains(gridXParam, gridYParam));
        }
        ...
        this.removeGridSections = function() {
            ...
            for (var i = 0; i < xParams.length; i++) {
                ...
                if (this.gridSectionManager.gridSections[x] != null && this.gridSectionManager.gridSections[x][y] != null) {
                    this.gridSectionManager.gridSections[x][y].parentNode.removeChild(this.gridSectionManager.gridSections[x][y]);
