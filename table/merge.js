var target = "div_temp" ; 
var opts = {correctBg:"green" , cursor:"default"}
var MergeTable ;
window.onload = function()
{
	MergeTable = (
		...
		{	
			...
			var Validation = {
				i_str_name : "验证" , 
				i_str_msg : "表格字符串格式不正确!" ,
				i_tid_msg : "无法根据id找到元素!",
			...
			if(typeof target == "string")
			{
				...
				if(temp_elem == null) alert("无法根据id或name获取元素!");
			}
			...
			for(var i = 0 , len = trs.length ; i <len ; i ++ )
			{
				var tds = trs[i].cells ? trs[i].cells : trs[i].getElementsByTagName("td") ; 
				{
					for(var j = 0 , l = tds.length ; j < l  ; j++)
					{
						...
						var rSpan = tds[j].rowSpan ; 
						if(rSpan > 1)
						{
							var countStep = 0 ; 
							tds[j+1] ? countStep += tds[j+1].rowSpan : null ; 
							for(var k = i + 1 ; k < i + rSpan ; k ++)
							{
								if(countStep <= rSpan)
								{
									var r_side = trs[k].cells ? trs[k].cells[tds[j].cellIndex] : trs[k].getElementsByTagName("td")[tds[j].cellIndex] ; 
									if(r_side)
									{
										countStep += r_side.rowSpan ; 
										CrossCellCollection.kVs.push(new KV(r_side , tds[j])) ; 
									}
								}
							}
						}
			...
			if(tableObj.attachEvent) tableObj.onselectstart = function(){event.returnValue = false; }
			if(tableObj.addEventListener) tableObj.addEventListener("selectstart" , function(e){e.preventDefault();})
