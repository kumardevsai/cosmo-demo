...
window.onload = function() {
	MergeTable = (
		function(table, target, opts) {
			...
			var MergeCellCollection = {
				kVs: [],
				push: function(k, vs, f) {
					var flag = false;
					for (var i = 0; i < this.kVs.length; i++) {
						if (this.kVs[i].K == k) {
							this.kVs[i].VS.push(vs);
							flag = true;
							break;
						}
					}
					if (flag == false) {
						this.kVs.push(new KVS(k, [vs], f));
					}
				}
			};

			function KVS(K, VS, flag) {
				this.K = K;
				this.VS = VS;
			...
			for (var i = 0, len = trs.length; i < len; i++) {
				var tds = trs[i].cells ? trs[i].cells : trs[i].getElementsByTagName("td"); {
					for (var j = 0, l = tds.length; j < l; j++) {
						if (tds[j].colSpan > 1) {
							var vs = [];
							for (var k = 0; k < tds[j].colSpan - 1; k++) {
								var insertTd = document.createElement("td");
								insertAfter(insertTd, tds[j]);
								vs.push(insertTd);
							}
							MergeCellCollection.push(tds[j], vs, true);
						}
						if (tds[j].rowSpan > 1) {
							for (var m = i + 1; m <= i + tds[j].rowSpan - 1; m++) {
								var vs = [];
								for (var n = tds[j].cellIndex - 1; n < tds[j].cellIndex + tds[j].colSpan - 1; n++) {
									var tm;
									if (tds[j].cellIndex == 0)
										break;
									if (!trs[m].childNodes[tds[j].cellIndex - 1]) {
										for (var v = 2; v < trs[m].childNodes.length; v++) {
											tm = trs[m].childNodes[tds[j].cellIndex - v];
											if (tm)
												break;
										}
									} else
										tm = trs[m].childNodes[tds[j].cellIndex - 1];
									var insertTd = document.createElement("td");
									insertAfter(insertTd, tm);
									vs.push(insertTd);
								}
								MergeCellCollection.push(tds[j], vs);
							}
						}
						tds[j].rowSpan = 1;
						tds[j].colSpan = 1;
			...
			for (var i = 0, len = trs.length; i < len; i++) {
				...
				Demissions2CellCollection.d2Cells[i] = [];
				for (var j = 0, l = td_len; j < l; j++) {
					if (td_len < Demissions2CellCollection.x_axis) {
						var find = false;
						if (CrossCellCollection.kVs.length > 0) {
							for (var m = 0; m < CrossCellCollection.kVs.length; m++) {
								if (CrossCellCollection.kVs[m].K === tds[j]) {
									var before_td = CrossCellCollection.kVs[m].V;
									var vertical_x = parseInt(before_td.getAttribute("altX")) + parseInt(before_td.colSpan);
									tds[j].setAttribute("altX", vertical_x);
									Demissions2CellCollection.d2Cells[i][vertical_x] = tds[j];
									find = true;
									break;
								}
							}
						...
						if (find == false) {
							if (tds[j - 1]) {
								var vertical_x = parseInt(tds[j - 1].getAttribute("altX")) + parseInt(tds[j - 1].colSpan);
								tds[j].setAttribute("altX", vertical_x);
								Demissions2CellCollection.d2Cells[i][vertical_x] = tds[j];
							} else {
								tds[j].setAttribute("altX", 0);
								Demissions2CellCollection.d2Cells[i][0] = tds[j];
							}
						}
					} else {
					...
					tds[j].setAttribute("altY", i);
					tds[j].innerHTML = "(" + tds[j].getAttribute("altY") + "," + tds[j].getAttribute("altX") + ")";
				}
			}
			**/
			// merge
			for (var i = 0; i < MergeCellCollection.kVs.length; i++) {
				var mergeTd = MergeCellCollection.kVs[i].K;
				for (var k = 0; k < MergeCellCollection.kVs[i].VS.length; k++) {
					for (var j = 0; j < MergeCellCollection.kVs[i].VS[k].length; j++) {
						MergeCellCollection.kVs[i].VS[k][j].parentNode.removeChild(MergeCellCollection.kVs[i].VS[k][j]);
					}
					if (k == 0) {
						if (MergeCellCollection.kVs[i].flag)
							mergeTd.colSpan = MergeCellCollection.kVs[i].VS[k].length + 1;
						else
							mergeTd.colSpan = MergeCellCollection.kVs[i].VS[k].length;
					}
				}
				mergeTd.rowSpan = MergeCellCollection.kVs[i].flag ? MergeCellCollection.kVs[i].VS.length : MergeCellCollection.kVs[i].VS.length + 1;
			}
			var Behavior = {
				correctBg: "#3478E5",
				cursor: "default",
			...
			Instance.prototype.mergeCell = function() {
				if (!checkCellsOutLine()) {
					alert("ÇëÖØÐÂÑ¡ÔñÐèÒªºÏ²¢µÄµ¥Ôª¸ñ!");
					clearCellField();
					return false;
				}
			...
			Instance.prototype.mergeExport = function(pattern) {
				...
				var rows = temp_table.rows ? temp_table.rows : temp_table.getElementsByTagName("tr");
				for (var i = 0, len = rows.length; i < len; i++) {
					var r_cells = rows[i].cells ? rows[i].cells : rows[i].getElementsByTagName("td");
					for (var j = 0, l = r_cells.length; j < l; j++) {
						r_cells[j].removeAttribute("altX");
						r_cells[j].removeAttribute("altY");
					}
				}
				if (pattern == "text") {
